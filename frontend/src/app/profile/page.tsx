...
export default function Profile() {
  ...
  const handleSubmit = async (e:any) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8881/api/v1/users/update-profile/${userId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({formData, token: "Bearer" +" "+ currentUser.data.accessToken}),
      });
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:8881/api/v1/users/delete/${userId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      request.resource.contentType.matches('image/.*') */}
        <Image
          src={""|| formData?.profile || currentUser?.profile || currentUser?.data.user.profile}
          alt='profile'
        ...
        <input
          ...
          defaultValue={currentUser?.username || currentUser?.data.user.username ||""}
          type='text'
          id='username'
        ...
        <input
          ...
          defaultValue={ currentUser?.email || currentUser?.data.user.email ||""}
          type='email'
          id='email'
          placeholder='Email'
          className='bg-slate-100 rounded-lg p-3 text-black'
